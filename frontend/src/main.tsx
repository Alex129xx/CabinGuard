...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="layout">
      <aside className="panel simulator"><h2>车辆与驾驶员</h2><Metric label="车速" value={`${state.vehicle.speed_kmh.toFixed(1)} km/h`} /><Metric label="驾驶时长" value={formatDuration(state.driver.driving_duration_minutes)} /><Metric label="雨刷" value={state.vehicle.wiper_on ? '已开启' : '已关闭'} />
        ...
        <div className="scenario-grid"><button onClick={() => scenario('commute')}>正常通勤</button><button onClick={() => scenario('rainy')}>雨天出行</button><button onClick={() => scenario('attention')}>注意力分散</button><button className="danger" onClick={() => scenario('fatigue')}>疲劳驾驶</button></div>
      </aside>
      <section className="center"><div className="map panel"><AmapRoute state={state} onParkPosition={setParkPosition}/><div className="map-top"><span>当前路线</span><b>{state.navigation.status === 'active' ? `导航中 · ${state.navigation.simulated_speed_kmh.toFixed(1)} km/h` : '停车状态：点击地图设置位置'}</b></div>{!import.meta.env.VITE_AMAP_JS_KEY && <div className="road"><span className="pin start">●</span><div className="route-line" style={{width: route ? '100%' : '0%'}} /><span className="car" style={{left: '8%'}}>▰</span><span className="pin end">★</span></div>}{route ? <div className="route-info"><b>{route.distance_km} km</b><span>预计 {route.duration_minutes} 分钟</span>{state.navigation.status !== 'idle' && <button onClick={cancelNavigation}>结束导航并清除路线</button>}</div> : <p>停车时可点击地图设置车辆位置，然后说“带我去虹桥站”</p>}</div>
        <div className="cabin-cards"><Card icon="♨" label="空调" value={`${state.cabin.temperature}℃ · ${state.cabin.climate_mode === 'off' ? '已关闭' : state.cabin.climate_mode}`} /><Card icon="♫" label="媒体" value={`${state.cabin.media_mode} · ${state.cabin.volume}%`} /><Card icon="▧" label="座椅" value={`加热 ${state.cabin.seat_heating} · 按摩 ${state.cabin.seat_massage}`} />{weather && <Card icon="☂" label="天气" value={`${weather.location || '当前位置'} · ${weather.temperature}℃ ${weather.weather} · 降水 ${weather.precipitation_probability}%${weather.cached ? '（缓存）' : ''}`} />}</div>
